...
public class Package extends Object {
	...
	public Package(Api api, Project project, String packageName) {
		super(api, project, "/source/" + encodeURI(project.getName()) + "/"
	...
	public Package branch() throws OSCException {
		...
		Project project = new Project(api, this.getHost(), "home:"
				+ api.getUsername() + ":branches:" + getProjectName());
	...
	public List<String> getFiles() throws OSCException {
		/**
		 * doesn't really throw the exception, as the information is obtained in
		 * refresh();
		 */
		return fileList;
	}
	...
	public void refresh(int rev) throws OSCException {
		...
		/* We treat _meta as an ordinary file */
		fileList.add(0, "_meta");
		...
		if (link_project != null && link_package != null) {
			linkTarget = getHost().getProject(link_project).getPackage(
					link_package);
		...
		if (linkTarget != null) {
			...
			try {
				xml = new XML(ins);
				NodeList nodeList = (NodeList) xml.evaluate("//patches/*",
