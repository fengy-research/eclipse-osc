...
public abstract class XMLFile extends Element {
	...
	public void save() throws CoreException {
		ByteArrayInputStream is = new ByteArrayInputStream(xml.toString()
				.getBytes());
		if (getFile().exists()) {
	...
	protected List<String> getList(String path) {
		try {
			NodeList nodeList = (NodeList) xml.evaluate(path,
	...
	protected Node getNode(String path) {
		try {
			...
			try {
				org.w3c.dom.Element ele = (org.w3c.dom.Element) xml.evaluate(
						path.substring(0, at), XPathConstants.NODE);
				if (childName.startsWith("@")) {
					Attr attr = xml.document.createAttribute(childName
							.substring(1));
